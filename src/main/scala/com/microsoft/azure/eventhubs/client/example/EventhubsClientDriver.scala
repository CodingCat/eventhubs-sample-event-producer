...
import com.microsoft.azure.eventhubs.client.example.ClientArgumentParser.ArgumentMap
...
object EventhubsClientDriver {
  ...
  def produceEventsSync(threadCount: Int, messageCountPerThread: Long, partitionId: String, inputOptions: ArgumentMap):
      Unit = {
  ...
  def main(inputArguments: Array[String]): Unit = {
    ...
    var executionModel: String = "sync"
    ...
    if(inputOptions.contains(Symbol(ClientArgumentKeys.executionModel)))
      executionModel = inputOptions(Symbol(ClientArgumentKeys.executionModel)).asInstanceOf[String]

    if(inputOptions.contains(Symbol(ClientArgumentKeys.connectionString)))
      connectionString = inputOptions(Symbol(ClientArgumentKeys.connectionString)).asInstanceOf[String]
    ...
    executionModel match {
      case "sync" =>
        produceEventsSync(threadCount, messageCountPerThread, partitionId, inputOptions)
      case "async" =>
    }
